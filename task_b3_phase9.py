"""
Task B3, Step 2.3: full 39-project LOPO + within-project, using
delayed_v2 labels and v2 (with-text) features.
"""
...
def main():
    print("=== Task B3: Phase 9 v3 (delayed_v2 labels), full 39-project LOPO ===")
    ...
    for i, p in enumerate(all_projects, 1):
        ...
        n_test = len(test_df)
        delay_rate = test_df["delayed_v2"].mean()

        clf = xgb.XGBClassifier(**PHASE7_PARAMS)
        clf.fit(train_df[STATIC_FEATURE_COLS_V2], train_df["delayed_v2"])
        proba = clf.predict_proba(test_df[STATIC_FEATURE_COLS_V2])[:, 1]
        pred = (proba >= 0.5).astype(int)
        try:
            cross_auc = roc_auc_score(test_df["delayed_v2"], proba)
        except ValueError:
            cross_auc = np.nan
        cross_f1 = f1_score(test_df["delayed_v2"], pred, zero_division=0)
        ...
        elif proj_train["delayed_v2"].nunique() < 2:
            within_skipped, skip_reason = True, "train split missing a class"
        elif proj_test["delayed_v2"].nunique() < 2:
            within_skipped, skip_reason = True, "test split missing a class"
        else:
            clf_w = xgb.XGBClassifier(**PHASE7_PARAMS)
            clf_w.fit(proj_train[STATIC_FEATURE_COLS_V2], proj_train["delayed_v2"])
            proba_w = clf_w.predict_proba(proj_test[STATIC_FEATURE_COLS_V2])[:, 1]
            pred_w = (proba_w >= 0.5).astype(int)
            within_auc = roc_auc_score(proj_test["delayed_v2"], proba_w)
            within_f1 = f1_score(proj_test["delayed_v2"], pred_w, zero_division=0)
    ...
    out_path = f"{REPORTS_DIR}/phase9_generalization_v3.csv"
...
def plot_full_generalization(df):
    ...
    ax.set_title("Phase 9 v3 (train+val-median labels): cross-project vs. within-project AUC, sorted by N",
                 color=ink_primary)
    ...
    fig.tight_layout()
    out_path = f"{REPORTS_DIR}/phase9_generalization_v3.png"
